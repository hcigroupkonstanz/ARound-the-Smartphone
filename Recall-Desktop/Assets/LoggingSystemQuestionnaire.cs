...
public class LoggingSystemQuestionnaire : MonoBehaviour {

	#region FIELDS

	// static log file names and formatters
	private static string LOGFILE_DIRECTORY = "log2csv_logfiles";
	private static string LOGFILE_NAME_BASE = "questionnaire_log_file.csv";
	...
	public LikertScale[] Questionnaire;
	public GameObject InfoText;
	public GameObject QuestionnaireView;
	public GameObject QuestionnaireDoneView;
	...
	public bool saveQuestionnaire(){
		bool complete = true;
		foreach(LikertScale question in Questionnaire){
			if(question.getAnswer()[1] == -1){
		...
		if(complete){
			foreach(LikertScale question in Questionnaire){
				writeMessageToLog("QuestionID: "+ question.getAnswer()[0]+ " Answer: " +question.getAnswer()[1]);
			}
			//QuestionnaireDoneView.SetActive(true);
			//QuestionnaireView.SetActive(false);
		}
		return true;
	}
	public void restartQuestionnaire(){
		//QuestionnaireDoneView.SetActive(false);
