...
public class LoggingSystem : MonoBehaviour {
	...
	void Start () {
		...
		if(this.activeLogging)
		{
			...
			this.logFile = Application.persistentDataPath+LOGFILE_DIRECTORY
				+ "/"
				...
				+ LOGFILE_NAME_BASE;
			this.logFileMap = Application.persistentDataPath+LOGFILE_DIRECTORY
				+ "/"
				+ System.DateTime.UtcNow.ToString(LOGFILE_NAME_TIME_FORMAT)
				//+ System.DateTime.UtcNow.AddHours(2.0).ToString(LOGFILE_NAME_TIME_FORMAT)	// manually adjust time zone, e.g. + 2 UTC hours for summer time in location Stockholm/Sweden
				+"_map"
			...
			File.Create(this.logFileMap).Dispose();
	...
	//opens the map log
	public void startMapLog()
	{
	
		if(this.activeLogging)
		{
			if(File.Exists(this.logFileMap))
			{
				twMap = new StreamWriter(this.logFileMap, true);
	...
	public void writeMapToLog(string message)
	{
		if(this.activeLogging)
		{
			if(File.Exists(this.logFileMap))
			{	
				if(twMap == null)
					twMap = new StreamWriter(this.logFileMap, true);
				twMap.WriteLine(Time.realtimeSinceStartup.ToString().Replace(",",".") + ";" +message);
				twMap.Flush();
			}
		}
	}
